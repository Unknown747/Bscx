...
exports.analyzeWhale = analyzeWhale;
const axios_1 = __importDefault(require("axios"));
const GECKO_BASE = 'https://api.geckoterminal.com/api/v2';
const GECKO_HEADERS = { Accept: 'application/json;version=20230302' };
// ── Fetch trades for a wallet across GeckoTerminal pools ───────────────────────
async function fetchWalletTrades(walletAddress, maxPools = 8) {
    const allTrades = [];
    try {
        // Fetch trending + new pools
        const [trendRes, newRes] = await Promise.allSettled([
            axios_1.default.get(`${GECKO_BASE}/networks/base/pools?sort=h24_volume_desc&page=1`, { headers: GECKO_HEADERS, timeout: 8000 }),
            axios_1.default.get(`${GECKO_BASE}/networks/base/pools?sort=pool_created_at_desc&page=1`, { headers: GECKO_HEADERS, timeout: 8000 }),
        ]);
        const pools = [
        ...
        await Promise.allSettled(unique.map(async (pool) => {
            const poolAddress = pool.attributes.address;
            try {
                const res = await axios_1.default.get(`${GECKO_BASE}/networks/base/pools/${poolAddress}/trades`, { headers: GECKO_HEADERS, timeout: 7000 });
                const trades = res.data?.data ?? [];
                for (const t of trades) {
                    const a = t.attributes ?? {};
                    if ((a.tx_from_address ?? '').toLowerCase() !== walletAddress.toLowerCase())
                        continue;
                    allTrades.push({
...
function calcSharpe(buys, sells) {
    if (buys.length < 2)
        return 0;
...
function calcEntryTiming(buys) {
    if (!buys.length)
        return 50;
    // Earliest buy timestamp in each pool vs the pool's oldest trade
    const byPool = new Map();
    for (const b of buys) {
        if (!byPool.has(b.poolAddress))
            byPool.set(b.poolAddress, []);
...
function calc7DayProfit(buys, sells) {
...
// ── Public API ────────────────────────────────────────────────────────────────
async function analyzeWhale(walletAddress) {
...
//# sourceMappingURL=whale-analyzer-pro.js.map