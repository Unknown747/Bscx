...
exports.getEthPriceSync = getEthPriceSync;
exports.getBestPool = getBestPool;
exports.getCachedTokenPrice = getCachedTokenPrice;
exports.batchGetPrices = batchGetPrices;
exports.prioritizeWallets = prioritizeWallets;
...
function getEthPriceSync() { return ethPriceUsd; }
// ── ETH price refresh (every 60s) ─────────────────────────────────────────────
async function refreshEthPrice() {
    try {
        const res = await axios_1.default.get('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd', { timeout: 5000 });
...
async function getCachedTokenPrice(tokenAddress) {
    const key = tokenAddress.toLowerCase();
    ...
    if (!pool?.poolAddress)
        return null;
    try {
        const res = await axios_1.default.get(`${GECKO_BASE}/networks/base/pools/${pool.poolAddress}`, { headers: GECKO_HEADERS, timeout: 6000 });
        ...
        const entry = { priceUsd, priceEth, timestamp: Date.now() };
...
// ── Batch price fetch ─────────────────────────────────────────────────────────
async function batchGetPrices(tokenAddresses) {
    ...
    for (const chunk of chunks) {
        await Promise.allSettled(chunk.map(async (addr) => {
            ...
            if (entry)
                result.set(addr.toLowerCase(), entry);
        }));
    }
    return result;
}
// ── Sort wallets by profitability (for prioritisation) ────────────────────────
function prioritizeWallets(wallets, limit = 5) {
    return [...wallets]
        .sort((a, b) => {
