...
exports.CopyTradeMonitor = void 0;
...
class CopyTradeMonitor extends events_1.EventEmitter {
    constructor() {
        super();
        this.wallets = [];
        ...
        this.whaleHoldings = new Map(); // walletAddr → Set<tokenAddr>
        this.WETH_BASE = '0x4200000000000000000000000000000000000006';
        this.wallets = [];
    }
    // ============ START MONITORING ============
    start() {
    ...
    async scanWallets() {
        ...
        for (const wallet of this.wallets) {
            if (!wallet.isActive || wallet.pendingValidation)
                continue;
            try {
    ...
    async getRecentTransactions(address) {
        try {
            const response = await axios_1.default.get(`https://base.blockscout.com/api/v2/addresses/${address}/transactions`, { params: { filter: 'from', limit: 10 }, timeout: 5000 });
    ...
    filterNewBuys(transactions, wallet) {
        const newBuys = [];
        const now = Date.now();
        for (const tx of transactions) {
            ...
            if (this.recentTrades.has(txHash))
                continue;
            if (txTime === 0)
                continue;
            if (now - txTime > 30000)
    ...
    isSellTransaction(tx) {
        ...
        if (input.startsWith('0x414bf389') && input.length >= 138) {
            if (ethValue > 0.000001)
                return false; // ETH-in = buy, not sell
            const tokenIn = ('0x' + input.slice(34, 74)).toLowerCase();
    ...
    filterNewSells(transactions, wallet) {
        const newSells = [];
        const now = Date.now();
        for (const tx of transactions) {
            if (!this.isSellTransaction(tx))
                continue;
            const txHash = tx.hash || tx.id;
            const txTime = this.getTxTimestamp(tx);
            if (this.recentTrades.has(`sell_${txHash}`))
                continue;
            if (txTime === 0)
                continue;
            if (now - txTime > 60000)
                continue; // only recent sells (1 min)
            newSells.push(tx);
    ...
    getTxTimestamp(tx) {
        ...
        if (tx.time)
            return new Date(tx.time).getTime();
        return 0;
    }
    ...
    async processCopyOpportunity(tx, wallet) {
        const tokenAddress = this.extractTokenAddress(tx);
        if (!tokenAddress)
            return;
        ...
        if (tokenInfo.pairCreatedAt) {
            const ageHours = (Date.now() - tokenInfo.pairCreatedAt) / 3600000;
            if (ageHours > 24) {
        ...
        if (this.CONFIG.SIMULATION_ENABLED) {
            try {
                simulation = await (0, whale_finder_1.simulateCopyTrade)(wallet.address, tokenAddress);
        ...
        if (simulation)
            console.log(`🔮 Sim: +${simulation.estimatedProfit}% est. | ${simulation.estimatedRisk} risk`);
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
        ...
        try {
            (0, whale_correlator_1.recordWhaleBuy)(wallet.address, wallet.name, tokenAddress, tokenInfo.symbol, copyAmount);
        }
        catch { /* silent */ }
    ...
    async quickSafetyCheck(tokenAddress) {
        try {
            const response = await axios_1.default.get(`https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=${tokenAddress}`, { timeout: 5000 });
    ...
    // ============ PUBLIC METHODS ============
    addWallet(address, name, pendingValidation = false) {
    ...
    recordTradeOutcome(walletAddress, profitPct) {
        if (!walletAddress)
            return;
        ...
        if (total >= this.CONFIG.MIN_TRADES_BEFORE_SCORE &&
...
exports.default = CopyTradeMonitor;
//# sourceMappingURL=copy-trade-monitor.js.map