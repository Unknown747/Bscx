...
class SwapExecutor extends events_1.EventEmitter {
    ...
    async detectVolumeCollapse(tokenAddress) {
        ...
        try {
            ...
            if (cached && cached.expiresAt > Date.now()) {
                ...
                const poolsRes = await axios_1.default.get(`${GECKO}/networks/base/tokens/${tokenAddress}/pools?page=1`, { headers: HDR, timeout: 5000 });
                const pools = poolsRes.data?.data ?? [];
                if (!pools.length)
                    return { collapsed: false, dropPct: 0 };
                const best = pools.reduce((b, p) => parseFloat(p.attributes?.reserve_in_usd ?? '0') >
                    parseFloat(b.attributes?.reserve_in_usd ?? '0') ? p : b, pools[0]);
                poolAddress = best.attributes?.address ?? null;
                if (!poolAddress)
                    return { collapsed: false, dropPct: 0 };
                this.vcPoolCache.set(key, { poolAddress, expiresAt: Date.now() + 120000 });
            }
            // ── Step 2: fetch last 6 × 1-minute candles ───────────────────────
            const ohlcvRes = await axios_1.default.get(`${GECKO}/networks/base/pools/${poolAddress}/ohlcv/minute?aggregate=1&limit=6&currency=usd`, { headers: HDR, timeout: 5000 });
    ...
    async checkPosition(tokenAddress, position) {
        ...
        if (!position.exitingNow && !position.volumeCollapseExited) {
            const holdMinsVC = (Date.now() - position.openedAt) / 60000;
            if (holdMinsVC >= this.CONFIG.VOLUME_COLLAPSE_MIN_HOLD_MINS) {
                try {
                    ...
                    if (collapsed) {
                        console.log(`📉 VOLUME COLLAPSE EXIT: ${position.tokenSymbol} — ` +
