...
// ─── Help text ────────────────────────────────────────────────────────────────
const HELP_TEXT = `⚡ <b>Base Sniper — Commands</b>\n` +
    `<code>${DIV}</code>\n\n` +
    `<b>📊 Info</b>\n` +
...
class TelegramBot {
    ...
    async poll() {
        if (!this.running)
            return;
        try {
            const res = await axios_1.default.get(`https://api.telegram.org/bot${this.token}/getUpdates`, {
                params: {
                    offset: this.offset,
                    timeout: 30,
            ...
            for (const update of updates) {
                ...
                if (update.message)
                    await this.handleMessage(update.message).catch(() => { });
                if (update.callback_query)
                    await this.handleCallback(update.callback_query).catch(() => { });
            }
        }
        catch {
    ...
    async handleMessage(msg) {
        ...
        switch (cmd) {
            ...
            case 'set':
                ...
                break;
            default:
                await this.send(`❓ Unknown command: <code>/${cmd}</code>\n\nType /help for the full list.`);
        }
    }
    // ─── Callback router (inline buttons) ────────────────────────────────────
    async handleCallback(cb) {
        if (!this.isAuthorized(cb))
            return;
    ...
    async cmdStatus() {
        ...
        const posLines = pos.length > 0
            ...
            : '  <i>None</i>';
        await this.sendKb(`⚡ <b>Base Sniper — Status</b>\n` +
            `<code>${DIV}</code>\n\n` +
        ...
        try {
            ...
            const tokenLines = tokens.length > 0
                ? tokens.slice(0, 8).map((t) => {
                    const val = t.valueUsd != null ? `$${t.valueUsd.toFixed(2)}` : 'N/A';
                    const chg = t.change24h != null ? `  ${pct(t.change24h)}` : '';
                    return `  • <b>${t.symbol}</b>: ${parseFloat(t.balance).toFixed(4)}${chg} — <i>${val}</i>`;
                }).join('\n')
                : '  <i>No tokens held</i>';
            await this.send(`💰 <b>Wallet Balance</b>\n` +
                `<code>${DIV}</code>\n\n` +
    ...
    async cmdPositions() {
        ...
        const pos = st.openPositions ?? [];
        if (pos.length === 0) {
            await this.send(`📂 <b>Open Positions</b>\n` +
                `<code>${DIV}</code>\n\n` +
        ...
        let livePnl = [];
        try {
            livePnl = await this.bot.getLivePnL();
        }
        catch { /* silent */ }
        const lines = pos.map((p) => {
            const live = livePnl.find((l) => l.tokenAddress === p.tokenAddress);
            const holdMs = p.openedAt ? Date.now() - p.openedAt : 0;
            const pnlStr = live?.profitPct != null ? pct(live.profitPct) : 'N/A';
            const multStr = live?.multiplier != null ? `${live.multiplier.toFixed(2)}x` : '';
            const icon = !live?.profitPct ? '⚪' : live.profitPct > 0 ? '📈' : '📉';
            const entryEth = p.amountIn ? (Number(p.amountIn) / 1e18).toFixed(5) : '?';
            return (`${icon} <b>${p.tokenSymbol || 'UNKNOWN'}</b>${multStr ? `  [${multStr}]` : ''}\n` +
    ...
    async cmdScreener() {
        ...
        if (signals.length === 0) {
            await this.send(`📡 <b>SmartScreener</b>\n` +
                `<code>${DIV}</code>\n\n` +
        ...
        const lines = top.map((s, i) => {
            ...
            return (`${icon} <b>#${i + 1} ${s.tokenSymbol}</b>  [${s.signal}]\n` +
        ...
        await this.send(`📡 <b>SmartScreener — Top Signals</b>\n` +
            `<code>${DIV}</code>\n\n` +
            `${lines}\n\n` +
            `<code>${DIV}</code>\n` +
    ...
    async cmdRisk() {
        ...
        const resetIn = resetInMs > 0 ? Math.ceil(resetInMs / 60000) : 0;
        const cooldownLine = rs.cooldownUntil > Date.now()
            ? `⏳ Cooldown: ${Math.ceil((rs.cooldownUntil - Date.now()) / 60000)}m remaining\n`
            : '';
        await this.send(`🛡️ <b>Risk Manager</b>\n` +
            `<code>${DIV}</code>\n\n` +
    ...
    async cmdHistory() {
        ...
        const tradeLines = recent.length === 0
            ? '<i>No trades yet.</i>'
            : recent.map((t) => {
                const pnlStr = t.profitPct != null ? pct(t.profitPct) : 'N/A';
                const icon = t.profitPct == null ? '⚪' : t.profitPct > 0 ? '✅' : '❌';
    ...
    async cmdSell(args) {
        ...
        if (target === 'all') {
            ...
            try {
                ...
                await this.send(`🔴 <b>All Positions Sold</b>\n\n` +
        ...
        const position = pos.find((p) => (p.tokenSymbol || '').toLowerCase() === target ||
            (p.tokenAddress || '').toLowerCase().startsWith(target));
        if (!position) {
            const names = pos.map((p) => p.tokenSymbol || fmtAddr(p.tokenAddress)).join(', ');
            await this.send(`❌ Position not found: <code>${target}</code>\n\n` +
        ...
        try {
            ...
            if (result.success) {
                await this.send(`✅ <b>Sell Executed</b>\n\n` +
    ...
    async cmdPause() {
        if (this.bot.isEmergencyStopped()) {
            await this.send('⚠️ Bot is already stopped.\n\nUse /resume to re-enable scanners.');
            return;
        }
        await this.send(`🛑 <b>Pausing all trading…</b>\n` +
            `<i>Selling open positions and stopping all scanners.</i>`);
        try {
            const result = await this.bot.emergencyStop();
            await this.sendKb(`🛑 <b>Bot Paused</b>\n` +
                `<code>${DIV}</code>\n\n` +
                `✅ All scanners stopped\n` +
    ...
    async cmdBlacklist() {
        ...
        if (bl.length === 0) {
            await this.send(`🚫 <b>Blacklist</b>\n` +
                `<code>${DIV}</code>\n\n` +
                `<i>Empty — no tokens blacklisted.</i>`);
            return;
        }
        const lines = bl.slice(0, 12).map((b) => {
            const hoursAgo = Math.round((Date.now() - b.addedAt) / 3600000);
            return `• <code>${b.address.slice(0, 20)}…</code>${b.label ? ` — ${b.label}` : ''} <i>(${hoursAgo}h ago)</i>`;
        }).join('\n');
        await this.send(`🚫 <b>Blacklist — ${bl.length} Tokens</b>\n` +
            `<code>${DIV}</code>\n\n` +
            `${lines}` +
    ...
    async cmdSet(args) {
        ...
        const val = args[1] ?? '';
        if (!key || !val) {
            await this.send(`⚙️ <b>Settings</b>\n` +
                `<code>${DIV}</code>\n\n` +
    ...
    async sendKb(text, reply_markup) {
        try {
            await axios_1.default.post(`https://api.telegram.org/bot${this.token}/sendMessage`, { chat_id: this.chatId, text, parse_mode: 'HTML', disable_web_page_preview: true, reply_markup }, { timeout: 8000 });
        }
        catch { /* silent */ }
    }
