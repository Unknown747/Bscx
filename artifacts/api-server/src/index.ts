...
function generateToken(): string {
    ...
    const sig       = crypto.createHmac('sha256', _sigSecret()).update(payload).digest('hex');
    return `${payload}.${sig}`;
}
...
function verifyToken(token: string | undefined): boolean {
    ...
    const expected  = crypto.createHmac('sha256', _sigSecret()).update(payload).digest('hex');
...
async function startBot() {
    await initDb();
    paperTrader.reloadState();  // restore paper positions & config now that DB is ready
...
app.post('/api/whale/scan', (_req: Request, res: Response) => {
...
app.get('/api/whale/scan-status', (_req: Request, res: Response) => {
    res.json({
...
app.post('/api/wallet-scan', requireAuth, async (_req: Request, res: Response) => {
    try {
...
app.get('/api/mempool', requireAuth, (_req: Request, res: Response) => {
    try {
...
app.get('/api/history/export.csv', requireAuth, (_req: Request, res: Response) => {
    try {
...
app.get('/api/screener/history', requireAuth, (req: Request, res: Response) => {
    try {
...
// ============ CONNECTION STATUS ============
app.get('/api/connection-status', (_req: Request, res: Response) => {
