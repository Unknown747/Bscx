...
export class AISniperBot extends EventEmitter {
    ...
    private async checkHoneypot(tokenAddress: string): Promise<{ safe: boolean; reason?: string; sellTax?: number; buyTax?: number }> {
        try {
            ...
            if (holderCount < 50 && holderCount > 0) return { safe: false, reason: `Too few holders: ${holderCount}` };
    ...
    private setupEventHandlers(): void {
        ...
        this.smartScreener.on('buy-signal', async (signal: ScreenerSignal) => {
            ...
            if (this.runtimeConfig.serialRuggerEnabled) {
                try {
                    const sr = await checkSerialDeployer(signal.tokenAddress, this.runtimeConfig.serialRuggerMaxDeploys, this.runtimeConfig.serialRuggerWindowHours);
            ...
            if (this.runtimeConfig.reputationEnabled) {
                try {
                    const deployer = await getTokenDeployer(signal.tokenAddress as string);
                    if (deployer) {
            ...
            const rawAnalysis = await this.ai.analyzeToken(tokenAddress, {
                liquidity:     signal.liquidityUsd / (getEthPriceSync() || 3000),
            ...
            if (this.shouldBuy(analysis)) {
                ...
                setTimeout(async () => {
                    try {
                        const pos = this.executor?.getOpenPositions().find(
        ...
        this.smartScreener.on('skipped', (d: { symbol: string; pairAddress: string; reason: string; priceChangeH1: number; ageMinutes: number }) => {
            this.addLog('info',
                `🧊 Dilewati: ${d.symbol}`,
                d.reason
            );
