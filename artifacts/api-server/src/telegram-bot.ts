...
export class TelegramBot {
    ...
    private async handleMessage(msg: any): Promise<void> {
        ...
        switch (cmd) {
            ...
            case 'approve':     await this.cmdApprove(args[0] ?? ''); break;
            case 'reject':      await this.cmdReject(args[0] ?? '');  break;
            case 'promote':     await this.cmdPromote(args[0] ?? ''); break;
    ...
    private async handleCallback(cb: any): Promise<void> {
        ...
        switch (action) {
            case 'approve':  await this.cmdApprove(addr);        break;
            case 'reject':   await this.cmdReject(addr);         break;
            case 'promote':  await this.cmdPromote(addr);        break;
    ...
    private async cmdCandidates(): Promise<void> {
        ...
        const lines = top.map((c: any, i: number) => {
            ...
            const hoursAgo = ((Date.now() - c.lastActiveMs) / 3_600_000).toFixed(1);
            return (
                `${stars(c.score)} <b>#${i + 1}</b>  Score: <b>${c.score}/100</b>\n` +
                `<code>${scoreBar}</code>\n` +
                `<code>${c.address}</code>\n` +
        ...
        const keyboard = top.map((c: any, i: number) => ([
            { text: `✅ Approve #${i + 1}`, callback_data: `approve:${c.address}` },
            { text: `❌ Reject #${i + 1}`,  callback_data: `reject:${c.address}`  },
        ]));

        await this.sendKb(
            `🐋 <b>Whale Candidates — ${pending.length} Pending</b>\n` +
            `<code>${DIV}</code>\n\n` +
            `${lines}\n\n` +
            `<code>${DIV}</code>\n` +
            `<i>Tap a button or /approve &lt;addr&gt; · /reject &lt;addr&gt;</i>`,
    ...
    private async cmdApprove(addr: string): Promise<void> {
        ...
        if (!full) { await this.send('❌ Candidate not found. Use the full address.'); return; }
    ...
    private async cmdReject(addr: string): Promise<void> {
        if (!addr) { await this.send('❌ Usage: /reject <address>'); return; }
        const full = this.resolveAddr(addr);
        if (!full) { await this.send('❌ Candidate not found.'); return; }
    ...
    private async cmdPromote(addr: string): Promise<void> {
        if (!addr) {
            await this.send(
                '❌ Usage: /promote <address>\n\n' +
        ...
        if (ok) {
            ...
            await this.send(
                `❌ Could not promote <code>${addr}</code>\n\n` +
                `Make sure the wallet is in the monitoring queue first (/approve <addr>).`
            );
        }
    }

    private async cmdScan(): Promise<void> {
        await this.send(
            `🔍 <b>Whale Scan Started</b>\n\n` +
    ...
    async sendWaitlistAlert(candidate: {
        ...
        await this.sendKb(
            `🐋 <b>New Whale Candidate</b>  (${candidate.index + 1}/${candidate.total})\n` +
            `<code>${DIV}</code>\n\n` +
    ...
    private resolveAddr(addr: string): string | null {
        if (addr.length >= 42) return addr;
        const match = this.bot.getPendingWhales().find(
            (c: any) => (c.address as string).toLowerCase().startsWith(addr.toLowerCase())
        );
